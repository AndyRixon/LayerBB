...
foreach ($directory as $t) {
    if (is_dir('../public/themes/' . $t)) {
        $set = ($LAYER->data['site_theme'] == $t) ? ' class="success" title="Theme is currently set to default."' : '';
        $themes .= '<tr' . $set . '>
                        <td>' . $t . '</td>
                        <td>
                          <div class="btn-group">
...
$themes = '';
foreach( $query as $t ) {
    $set = ($LAYER->data['site_theme'] == $t['id']) ? ' class="success" title="Theme is currently set to default."' : '';
    $themes .= '<tr' . $set . '>
                  <td>' . $t['theme_name'] . '</td>
                    <td>
                      <div class="btn-group">
