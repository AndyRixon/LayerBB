...
class LAYER_Session
{
    ...
    public function check()
    {
        ...
        if (isset($_SESSION['LayerBB_sess']) or isset($_COOKIE['LayerBB_sess'])) {
            $id = (isset($_SESSION['LayerBB_sess'])) ? $_SESSION['LayerBB_sess'] : $_COOKIE['LayerBB_sess'];
    ...
    public function clear()
    {
        ...
        $time_cookie = strtotime(LAYER_SESSION_TIMEOUT . ' seconds ago');
    ...
    public function assign($email, $remember = false, $facebook = false)
    {
        ...
        if ($facebook) {
            setcookie('LayerBB_facebook', true, time() + LAYER_SESSION_TIMEOUT, '/', NULL, isset($_SERVER['HTTPS']), true);
        }
        ...
        if ($remember) {
            ...
            if ($insert > 0) {
                return setcookie('LayerBB_sess', $session_id, time() + LAYER_SESSION_TIMEOUT, '/', NULL, isset($_SERVER['HTTPS']), true);
            } else {
                return false;
            }
            ...
            $insert = $MYSQL->query("INSERT INTO {prefix}sessions (session_id, logged_user, session_type, session_time) VALUES (:session_id, :logged_user, :session_type, :session_time)");
            if ($insert > 0) {
                $_SESSION['LayerBB_sess'] = $session_id;
                return true;
            } else {
                return false;
            }
    ...
    public function remove()
    {
        ...
        if (isset($_SESSION['LayerBB_sess'])) {
            $MYSQL->bind('session_id', $_SESSION['LayerBB_sess']);
            $MYSQL->query("DELETE FROM {prefix}sessions WHERE session_id = :session_id");
            session_destroy();
        } else {
            $MYSQL->bind('session_id', $_COOKIE['LayerBB_sess']);
            $MYSQL->query("DELETE FROM {prefix}sessions WHERE session_id = :session_id");
            return setcookie('LayerBB_sess', '', time() - 3600, '/', NULL, isset($_SERVER['HTTPS']), true);
        }
    }
